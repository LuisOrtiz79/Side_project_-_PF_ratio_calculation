...
#########################################
#Extra for checkbox for more than one data
table_var = tk.IntVar()
cb_filter = tk.Checkbutton(mainframe, text='Use an external table for more than one data', variable=table_var, onvalue=1, offvalue=0)
cb_filter.grid(row=2, column=1)
#########################################
...
#######################################
# For reading the dataset and the number of results it wants
tk.Label(mainframe, text="How many data would want to calculate(input >= 2):").grid(row=curr_row, column=0)
entry_data = tk.Entry(mainframe)
entry_data.grid(row=curr_row, column=1)
curr_row += 1
######################################
...
def predict():
    ...
    #########################
    # In case of more than one inputs
    else:
        tk.Label(mainframe, text='                                ').grid(row=14, column=1) # Clear text
        in_features = read_input()
        model = load_model()
        ...
        # Regression Model predicts PaO2 Value
        i = table['SaturationSpO2']
        j = table['FiO2']
        z = table['PEEP']
        # Would do the loop for x time that user asks for
        for x in range(int(entry_data.get())):
            pred = np.expand_dims(pred, axis=0)
            pred = output_scaler.inverse_transform(pred).squeeze()
            pred = np.round(pred, decimals=2)
            pred_var.set('Predicted PaO2 Value: \n' + str(x) +  "\t" + str(pred))
            print('Prediction PaO2 Value:',str(x), pred)
    #############################
...
# Read input entries
table = pd.read_excel("tblLabVitalsVent.xlsx") # Reads the excel file
def read_input():
    if table_var.get() == 0:
        inputs = []
        ...
        # Inserts in the array for x value given in the entry
        x = 0
        while x is not int(entry_data.get()):
            inputs.append(float(i[x]))
            inputs.append(float(j[x]))
            inputs.append(float(z[x]))
            if feature_3_var.get() == 0:
                inputs.append(float(entry_vt.get()))
                inputs.append(float(entry_map.get()))
                inputs.append(float(entry_temp.get()))
